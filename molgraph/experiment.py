...
class ArgumentParser:
    def __init__(self):
        ...
        self.parser.add_argument('-m', '--model', choices=['GAT', 'GIN'], default='GAT', type=str)
        ...
        self.parser.add_argument('--fold', default=5, type=int, help='fold')
        ...
        self.parser.add_argument('--heads', default=2, type=int, help='attention heads')
        ...
        ### Training
        self.parser.add_argument('--batch_size', default=64, type=int, help='train batch size')
        ...
        self.parser.add_argument('--patience', type=int, default=20, help='patience for earlystopping')
        ...
        # —— KAN  —— 
        self.parser.add_argument('--use_kan_readout', action='store_true', default=False,
                                 help='replace final linear readout with KANLinear')
        self.parser.add_argument('--kan_grid_size', type=int, default=4,
                                 help='Fourier grid size used by KANLinear readout')
        self.parser.add_argument('--kan_readout_norm', type=str, default='none',
                                 choices=['none', 'batchnorm', 'layernorm'],
                                 help='Optional normalization applied after KAN readout')
...
def set_experiment_name(args, hyper=False):
    ...
    if isinstance(args.reduced, str):
        reduced_list = args.reduced if args.reduced else 'full'
    else:
        reduced_list = '_'.join(args.reduced) if len(args.reduced) > 0 else 'full'

    model_tag = args.model
    if getattr(args, 'use_kan_readout', False):
        # 区分使用 KAN 读出的实验目录，方便与 baseline 对比
        model_tag = f"{model_tag}_KA"
    
    if hyper:
        log_folder_name = os.path.join(*[args.file, model_tag+'_'+args.schema+'_'+reduced_list, 'hyper', f"{ts}"])
    else:
        log_folder_name = os.path.join(*[args.file, model_tag+'_'+args.schema+'_'+reduced_list, f"{ts}"])
    ...
    # 为可解释性可视化预留独立目录，便于 KANVisualizer 输出到 results 下
    vis_dir = os.path.join('./dataset/{}/visualizations'.format(log_folder_name), f"{exp_name}")
    os.makedirs(vis_dir, exist_ok=True)

    # 记录路径信息，便于在训练后生成 KAN 可解释性报告
    args.log_folder_name = log_folder_name
    args.exp_name = exp_name
    args.visualization_dir = vis_dir
