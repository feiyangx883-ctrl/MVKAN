...
class SubstructureGraph(MoleculeGraph):
    def __init__(self, smiles, tokenizer, normalize=True):
        self.smiles, self.mol = self._parse_smiles(smiles)
    ...
    def _parse_smiles(self, smiles):
        ...
        if mol is None:
            raise ValueError(f"Invalid SMILES provided for SubstructureGraph: {smiles}")
        canonical_smiles = Chem.MolToSmiles(mol)
        canonical_mol = Chem.MolFromSmiles(canonical_smiles)
        if canonical_mol is None:
            raise ValueError(f"Failed to canonicalize SMILES for SubstructureGraph: {smiles}")
    ...
    def generate_morgan(self, mol, radius=2, nBits=1024):
        ...
        return np.asarray(morgan.ToList(), dtype=np.float32)
    ...
    def getCliqueFeatures_substructure(self, clique, edges, clique_idx, cliques_attr, edges_attr, mol):
        ...
        if substructure not in self._substructure_cache:
            substructure_mol = Chem.MolFromSmiles(substructure)
            if substructure_mol is None:
                raise ValueError(f"Invalid substructure SMILES encountered: {substructure}")
            self._substructure_cache[substructure] = (
                Chem.MolToSmiles(substructure_mol),
                self.generate_morgan(substructure_mol)
            )
        canonical_smiles, morgan_features = self._substructure_cache[substructure]
        self.cliques_smiles[clique_idx] = [canonical_smiles]
        return morgan_features.copy()
    ...
    def mol_to_graph(self, mol, cliques, edges, cliques_attr, edges_attr, normalize=True):
        ...
        for idx in range(len(cliques)):
            ...
            if normalize:
                total = float(np.sum(clique_features))
                if total != 0:
                    clique_attr.append((clique_features / total).tolist())
                    continue
            clique_attr.append(clique_features.tolist())
        ...
        for idx in range(len(edges)):
            ...
            if normalize:
                total = float(np.sum(cliqueedge_features))
                if total != 0:
                    cliqueedge_attr.append((cliqueedge_features / total).tolist())
                    continue
            cliqueedge_attr.append(cliqueedge_features.tolist())
