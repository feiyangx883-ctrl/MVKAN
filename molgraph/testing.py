        ...
        with torch.no_grad():
            if self.args.graphtask == 'regression':
                ...
                for data in test_loader:
                    ...
                    # Compute loss
                    y_target = data.y.to(out.dtype)
                    loss += loss_fn(out, y_target).item() * getNumberofSmiles(data)
                ...
                y_test = np.array(y_test, dtype=float)  # [N, num_tasks]
                y_pred = np.array(y_pred, dtype=float)  # [N, num_tasks]
                ...
                for task_idx in range(num_tasks):
                    ...
                    y_score = y_pred[:, task_idx]
                    
                    # Filter out NaN values (missing labels)
                    mask = ~np.isnan(y_true)
                    if mask.sum() > 0:
                        try:
                            fpr, tpr, threshold = metrics.roc_curve(y_true[mask], y_score[mask])
                            auc = metrics.auc(fpr, tpr)
                            auc_per_task.append(auc)
                            print(f'Task {task_idx} AUC: {auc:.4f}')
                        except:
                            print(f'Task {task_idx}: Unable to compute AUC')
                            pass
                
                # Compute average AUC
                avg_auc = np.mean(auc_per_task) if len(auc_per_task) > 0 else 0.0
                print(f'Average AUC across tasks: {avg_auc:.4f}')
        ...
        with torch.no_grad():
            if self.args.graphtask == 'regression':
                ...
                for data in test_loader:
                    ...
                    y_target = data.y.to(out.dtype)
                    loss += loss_fn(out, y_target).item()
                    y_test.extend((data.y.cpu()).detach().numpy())
                    y_pred.extend((pred.cpu()).detach().numpy())
                    x_embed.extend(self.model_test.last_mol_embedding.detach().cpu().numpy())
